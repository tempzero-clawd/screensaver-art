...
*Why it's a hard rule, not a preference:* a committed blob is effectively permanent. Deleting it
in a later commit doesn't shrink the repo — the object stays in history, every future `clone`
pays for it forever, and getting it out means rewriting history (or, for a blob that reached a
PR, asking GitHub Support, since a closed PR's `refs/pull/N/head` keeps it alive even after the
branch is deleted). This repo generates media *nightly*, so the failure mode compounds fast.

*Where media goes instead:*
- **Product art** → the Cloudflare R2 bucket (`screensaver-assets`, `gallery/` prefix); `gallery.json` stores the URL. See *Add new art pieces*.
- **Generated/derived assets** → gitignored build output (e.g. `marketing/out/`), regenerated by script.
- **Genuinely needed in-repo** (app icons, a brand mark, `marketing/assets/url-pill.png`) → **ask, then commit** — these are rare, tiny, and load-bearing.
